                  ...
                  <tr>
                    <td width="48" valign="top">
                      <a href="#">
                        <img
                          :src="
                            require(`@/assets/${$store.state.user.userImg}`)
                          "
                          class="avatar"
                          border="0"
                          align="default"
        ...
        <div class="Main">
          ...
          <div class="box" style="border-bottom:0px;">
            <div class="header">
              <div class="fr">
                <img
                  :src="require(`../../assets/${info.user.userImg}`)"
                  class="size"
                />
              </div>
              <div class="sep10"></div>
			   <el-button type="primary" class="custom-button"  @click="toggleFollow">
			      {{ isFollowed ? '已关注' : '关注' }}
			    </el-button>
				<el-button type="danger" v-show="$store.state.user.userId === info.user.userId" round size="small" @click="deleteItem">删除本帖子</el-button>
...
<script>
import { HotNumbe } from "@/api";
import { newcomment } from "@/api";
import { getcomment } from "@/api";
import { findartbyuserid } from "@/api";
import { GuanzhuStart } from "@/api";
import { GuanZhu } from "@/api";
import { UnGuanZhu } from "@/api";
...
export default {
  ...
  created() {
	...
	HotNumbe(this.info.article.artId).then(res =>{
		console.log(res)
		console.log("浏览");
	})
  ...
  methods: {
	  deleteItem(){
		  delet(this.info.article.artId).then(res=>{
			  console.log(res)
			  alert("删除成功");
			   this.$router.push("/");
		  })
	  ...
	  GuanzhuStart(this.$store.state.user.userId,this.info.user.userId).then(res=>{
	  ...
	  toggleFollow() {
		   if(!this.isFollowed){
			   GuanZhu(this.$store.state.user.userId,this.info.user.userId).then(res=>{
				   this.isFollowed = res.data.data;
				   console.log(res.data.data)
			   })
		   }else{
			   UnGuanZhu(this.$store.state.user.userId,this.info.user.userId).then(res=>{
				   this.isFollowed = res.data.data
			   })
		   }
