        ...
        <div id="Rightbar">
          ...
          <div class="box">
            ...
            <nav class="level is-mobile">
              ...
              <div class="level-item has-text-centered">
                <div>
                  <p class="heading" @click="numfensi">粉丝</p>
                  <p>{{this.fensi}}</p>
                </div>
              </div>
              <div class="level-item has-text-centered">
                <div>
                  <p class="heading" @click="numGuanzhu">你关注的人数</p>
                  <p>{{this.GuanZhu}}</p>
                </div>
              </div>
              <div class="level-item has-text-centered">
          ...
          <div class="box">
            ...
            <div class="inner">
              ...
              <div class="box" v-for="(item, i) in mypost" :key="i">
				   <el-button type="danger" round size="small" @click="deleteItem(i)">删除</el-button>
                <article class="media">
...
export default {
  ...
  created() {
    ...
    findartbyuserid(this.$store.state.user.userId).then(res => {
      ...
      const { data } = res.data;
      this.mypost = data.article;
      this.mypostnum = data.article.length;
	  this.like = data.like;
	  this.fensi = data.fensi
	  this.GuanZhu = data.guanzhu
    });
  },
  methods:{
	  deleteItem(i){
		  const articla=this.mypost[i]
		  delet(articla.artId).then(res=>{
		  			  console.log(res)
		  			  alert("删除成功");
		  			  location.reload();
		  })
